...
def run_experiment(config,modality):
    ...
    for run in range(initial_run,config['runs']):
        ...
        # log
        print("Run:{} | Accuracy {:.2f} | Mean Accuracy:{:.2f} | Std Dev:{:.2f} | MVO Accuracy:{:.2f} | Std Dev:{:.2f} | MVY Accuracy:{:.2f} | Std Dev:{:.2f} \tT:{:.2f}min\tE:{:.2f}".format(run+1,test_acc, np.mean(run_acc), np.std(run_acc),np.mean(run_mvo), np.std(run_mvo),np.mean(run_mvy), np.std(run_mvy),(train_time -run_time)/60,(end_eval_time -init_eval_time)/60))
