...
def compute_prototypes(support: torch.Tensor, targets: int,
                       classes: int = 10) -> torch.Tensor:
    ...
    for epoch in range(1, num_epochs + 1):
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                # compute prototypes
                sorted_targets_memory, sorted_indices = torch.sort(
                    targets_memory)
                sorted_support = embedding_support[sorted_indices]
                prototypes = compute_prototypes(sorted_support,
                                                sorted_targets_memory)
                prototypes = prototypes.to(device)

                # get output
                distances = utils.vector_distance(embedding_input, prototypes,
                                                  'cosine')
...
def eval_model(model: torch.nn.Module, loader: torch.utils.data.DataLoader,
    ...
    """ Method to evaluate a pytorch model that has a Memory Wrap variant
        as last layer. The model takes an image and a memory samples as input
...
def run_experiment(config: dict):
    ...
    save = config['save']
    path_saving_model = f"../models/{dataset_name}/proto/{config['model']}" + \
                        f"/{config['train_examples']}/"
    ...
    for run in range(initial_run, config['runs']):
        ...
        utils.set_seed(run)
        torch.cuda.init()
        model = utils.get_model(config['model'], num_classes,
                                model_type=modality)
        ...
        train_loader, _, test_loader, mem_loader = utils.get_loaders(
            config, run, balanced=True)
        train_time = time.time()
        model = train_model(model, loaders=[train_loader, mem_loader],
                            optimizer=optimizer, scheduler=scheduler,
                            loss_criterion=loss_criterion,
                            num_epochs=config[dataset_name]['num_epochs'],
                            device=device)
        init_eval_time = time.time()
        best_acc, _ = eval_model(model, test_loader, mem_loader,
        ...
        if save and path_saving_model:
            ...
            torch.save({'model_state_dict': model.state_dict(),
                        ...
                        'mem_examples':
                            config[config['dataset_name']]['mem_examples'],
