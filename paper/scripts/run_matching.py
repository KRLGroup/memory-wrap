...
class MatchingNetwork(nn.Module):
    def __init__(self, encoder: torch.nn.Module, fce: bool, num_input_channels: int,
        ...
        # Arguments:
            ...
            num_input_channels: Number of color channels the model expects input data
                to contain.
            lstm_layers: Number of LSTM layers in the bidrectional LSTM g that embeds
                the support set (fce = True)
            lstm_input_size: Input size for the bidirectional and Attention LSTM. This is
                determined by the embedding dimension of the few shot encoder which is in
                turn determined by the size of the input data.
...
class BidrectionalLSTM(nn.Module):
    def __init__(self, size: int, layers: int):
        ...
        # Arguments
            size: Size of input and hidden layers. These are constrained to be the
                same in order to implement the skip connection described in Appendix A.2
...
class AttentionLSTM(nn.Module):
    def __init__(self, size: int, unrolling_steps: int):
        ...
        # Arguments
            size: Size of input and hidden layers. These are constrained to be the
                same in order to implement the skip connection described in Appendix A.2
            unrolling_steps: Number of steps of attention over the support set to compute.
                Analogous to number of layers in a regular LSTM
        """
    ...
    for epoch in range(1, num_epochs + 1):
        for batch_idx, (data, y) in enumerate(train_loader):
            ...
            # in appendix A.2 of the paper. g takes the form of a bidirectional LSTM with a
                ...
                embedding_support, _, _ = model.g(embedding_support.unsqueeze(1))
...
def run_experiment(config:dict):
    ...
    save = config['save']
    path_saving_model = '../models/{}/matching/{}/{}/'.format(dataset_name, config['model'],config['train_examples'])
    ...
    for run in range(initial_run,config['runs']):
        ...
        utils.set_seed(run)
        torch.cuda.init()
        model_encoder = utils.get_model(config['model'],num_classes,model_type=modality)
        model_encoder = model_encoder.to(device)
        num_input_features = model_encoder.linear.in_features
        model_encoder.linear = torch.nn.Identity()
        fce = True
        model = MatchingNetwork(model_encoder, fce, num_input_channels=3, lstm_layers=1,
                        lstm_input_size=num_input_features, unrolling_steps=2, device=device)
        ...
        # get dataset
        train_loader, _, test_loader, mem_loader = utils.get_loaders(config,run,balanced=False)
        train_time = time.time()
        model = train_model(model, [train_loader,mem_loader], optimizer, scheduler,
                        loss_criterion, config[dataset_name]['num_epochs'], device=device) 
        init_eval_time = time.time()
        best_acc, _ = eval(model,test_loader, mem_loader,loss_criterion,device)
