...
from wrappers import densenet,googlenet,mobilenet,resnet,shufflenet,efficientnet

# user flags
absl.flags.DEFINE_string("dir_models", None, "dir path where models are stored")
absl.flags.DEFINE_string("dir_dataset", '../datasets/', "dir path where datasets are stored")
absl.flags.DEFINE_enum("metric",None,['input','prediction'], "Metric to be used")
...
def get_ebe_index(sorted_indices, pred_v, pred_t):
    ...
    in_memory = (sorted_pred == pred_t.unsqueeze(dim=1).repeat(1,sorted_indices.shape[1]))
...
def get_model_wrapper(model_name: str, num_classes:int , model_type:str) -> torch.nn.Module:
    ...
    if model_type not in ['memory','encoder_memory','std']:
        raise ValueError(f"modality (model type) must be one of " \
                         f"[\'memory\',\'encoder_memory\',\'std\'], not {model_type}.")
...
def run_experiment(path:str,dataset_dir:str, metric:str):
    ...
    for _, name_model in enumerate(sorted(list_models)):
        ...
        model = get_model_wrapper(model_name,checkpoint['num_classes'],model_type=modality)
        ...
        train_loader, _, test_loader, mem_loader = load_dataset(dataset_dir, batch_size_train=256,
                batch_size_test=256, batch_size_memory=mem_examples, size_train=train_examples,
                seed=run)
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                if modality == 'std':
                    ...
                    memory, _ = next(iter(mem_loader))
                    memory = memory.to(device)
                    logits, encoded_imgs, memory_weights = model(imgs,memory,return_weights=True)
                pred = torch.argmax(logits, axis=1)
                ...
                chp_kNN = KNeighborsClassifier(n_neighbors=1, algorithm="brute")
                dkNN = KNeighborsClassifier(n_neighbors=1, algorithm="brute")
                ...
                p_nns_chp = chp_kNN.kneighbors(x_cont.cpu().numpy(), n_neighbors=n_neighbors,
                                               return_distance=False)
                p_nns_dknn = dkNN.kneighbors(encoded_imgs.cpu().numpy(), n_neighbors=n_neighbors,
                                             return_distance=False)
                ...
                sorted_memory_weights_idx = torch.argsort(memory_weights,descending=True)

                # Memory Explanation By Examples (ebe)
                index_of_ebe = get_ebe_index(sorted_memory_weights_idx, exp_pred, pred)
                ...
                random_neighbors = torch.stack([torch.randperm(len(p_nns_chp[row])) for row
                ...
                # Compute scores
                mem_differences.append(loss_criterion(log_mem,considered_pred).item())
                chpt1_differences.append(loss_criterion(log_chp1,considered_pred).item())
                dknn1_differences.append(loss_criterion(log_dknn1,considered_pred).item())
                random_difference.append(loss_criterion(log_random,considered_pred).item())


                # Log stuff
                print(f"({i / len(test_loader):.2f})\t\tchp:{np.mean(chpt1_differences):.4f}\t" \
        ...
        print(f"Run:{run+1} | " \
              f"CHP {np.mean(run_chpt1_differences):.4f} Std Dev:{np.std(run_chpt1_differences):.4f} | " \
              f"KNN*:{np.mean(run_dknn1_differences):.4f} Std Dev:{np.std(run_dknn1_differences):.4f}  | " \
              f"MEM:{np.mean(run_mem_differences):.4f} Std Dev:{np.std(run_mem_differences):.4f} | " \
              f"RANDOM {np.mean(run_random_differences):.4f}  Std Dev:{np.std(run_random_differences):.4f}")
