...
class Encoder_Wide_ResNet(nn.Module):

    def __init__(self, depth, widen_factor, num_classes, dropout):
        super(Encoder_Wide_ResNet, self).__init__()
        assert ((depth-4)%6==0), 'Wide-resnet depth should be 6n+4'
        n = int((depth-4)/6)
        k = widen_factor
        nStages = [16, 16*k, 32*k, 64*k]
        ...
        self.layer2 = self._wide_layer(Wide_Basic, nStages[2], n, 2, dropout)
        self.layer3 = self._wide_layer(Wide_Basic, nStages[3], n, 2, dropout)
        self.mw = EncoderMemoryWrapLayer(nStages[3], num_classes)
        self.classifier = nn.Sequential(
            nn.BatchNorm2d(nStages[3], momentum=0.9),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(8),
...
class Memory_Wide_ResNet(nn.Module):

    def __init__(self, depth, widen_factor, num_classes, dropout):
        super(Memory_Wide_ResNet, self).__init__()
        assert ((depth-4)%6==0), 'Wide-resnet depth should be 6n+4'
        n = int((depth-4)/6)
        k = widen_factor
        nStages = [16, 16*k, 32*k, 64*k]
        ...
        self.layer2 = self._wide_layer(Wide_Basic, nStages[2], n, 2, dropout)
        self.layer3 = self._wide_layer(Wide_Basic, nStages[3], n, 2, dropout)
        self.mw = MemoryWrapLayer(nStages[3], num_classes)
        self.classifier = nn.Sequential(
            nn.BatchNorm2d(nStages[3], momentum=0.9),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(8),
...
def wrn28_2(num_classes=10, dropout=0.3):
    return Wide_ResNet(28, 2, num_classes, dropout)

def encoder_wrn28_10(num_classes=10, dropout=0.3):
    return Encoder_Wide_ResNet(28, 10, num_classes, dropout)


def encoder_wrn28_2(num_classes=10, dropout=0.3):
    return Encoder_Wide_ResNet(28, 2, num_classes, dropout)

def memory_wrn28_10(num_classes=10, dropout=0.3):
    return Memory_Wide_ResNet(28, 10, num_classes, dropout)


def memory_wrn28_2(num_classes=10, dropout=0.3):
    return Memory_Wide_ResNet(28, 2, num_classes, dropout)