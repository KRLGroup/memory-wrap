...
parser.add_argument("--model-name", default="encoder_vit", help="[vit]", type=str)
...
def get_loaders_args(dataset_name, dataset_dir, batch_size_train, batch_size_test, train_examples, mem_examples, seed=42):
    if dataset_name == 'c10':
        name = 'CIFAR10'
    elif dataset_name == 'svhn':
        name = 'SVHN'
    load_dataset = getattr(datasets, 'get_'+name)
    loaders = load_dataset(dataset_dir,batch_size_train=batch_size_train, batch_size_test=batch_size_test,batch_size_memory=mem_examples,size_train=train_examples,seed=seed)
    return loaders
...
class Net(pl.LightningModule):
    ...
    #     loaders = {"data": test_dl, "memory": mem_loader}
    #     combined_loaders = CombinedLoader(loaders, "max_size_cycle")
    #     return combined_loaders
...
if __name__ == "__main__":
    ...
    for run in range(0,15):
        ...
        train_dl = torch.utils.data.DataLoader( train_data, batch_size=args.batch_size,pin_memory=True, shuffle=True, drop_last=True,worker_init_fn=run)

        test_dl = torch.utils.data.DataLoader(test_ds, batch_size=args.eval_batch_size,pin_memory=True,worker_init_fn=run)

        mem_loader = torch.utils.data.DataLoader(train_data, batch_size=100,pin_memory=True, shuffle=True, drop_last=True,worker_init_fn=run)
        ...
        net = Net(args)
        trainer = pl.Trainer(precision=args.precision,fast_dev_run=args.dry_run, gpus=args.gpus, benchmark=args.benchmark, logger=logger, max_epochs=args.max_epochs,enable_model_summary=False, progress_bar_refresh_rate=refresh_rate)
        trainer.fit(model=net,train_dataloaders=combined_training_loaders,val_dataloaders=combined_test_loaders)
        ...
        print("Run:{} | Mean Accuracy:{:.4f} | Std Dev Accuracy:{:.4f}\t".format(run+1,np.mean(accuracies), np.std(accuracies)))